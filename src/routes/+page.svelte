<script>
	...
	let promise = supabase.from('countries').
	select();
	let names = supabase.from('rabbits').select();
	let name ="";
	async function sendName(){
		const response =await fetch('/api/sendName',{method: "POST", headers: {"Content-Type":"application/json",},
...
<ul>
	...
	<li><a href="api/hello/Paul">Link zur API mit QRoute-Parameter</a></li>
</ul><form>
